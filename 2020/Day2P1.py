...
def GetKeyParameters(x):
	params = x.split(":")
	ch = params[0][-1]
	word = params[1].strip()
	minmax = params[0][0:-2].split("-");
	return (minmax[0],minmax[1],ch,word)
...
for x in lst:
	(min, max,chValue,word)  = GetKeyParameters(x)
	#print("min",min, "max",max, "chValue",chValue,"word->",{word},"<-");
	charsCounter = dict();
	#charsCounter = countChars(word);
	charsCounter[chValue] = word.count(chValue)
	
	if chValue in charsCounter:
		if charsCounter[chValue] >= int(min) and charsCounter[chValue] <= int(max):
			print(x, " matched policy")
